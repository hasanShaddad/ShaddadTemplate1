    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                //we edit the action to incloade email and confirm the regesteration
                try
                {
                    ViewBag.To = model.UserName;
                    string confirmationToken =
              WebSecurity.CreateUserAndAccount(model.UserName, model.Password, new { Email = model.Email }, true);
                    Mail.SendMail(model.UserName, model.Password, model.Email, confirmationToken);
        ...
        [Authorize(Roles = "Admin")]//صلاحية الدخول للمطورين فقط
        [ValidateAntiForgeryToken]
        public ActionResult RemoveAdmin(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                // Attempt to register the user
